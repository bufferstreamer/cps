...
//            throw new BusinessException("未开通折扣或折让权限");
        ...
        if (StringUtils.isNotNull(whOutboundOrder.getDiscountRate()) && !WhWarehousingOrderType.TRANSFER.getCode().equals(whOutboundOrder.getOrderType()) && whOutboundOrder.getDiscountRate().compareTo(BigDecimal.ONE) == -1) {//a小于b
            if (basisDiscountPermission.getMinDiscountRate().compareTo(whOutboundOrder.getDiscountRate()) == 1) {
                throw new BusinessException("已超过最低折扣权限:" + basisDiscountPermission.getMinDiscountRate());
            }
        }
        if (StringUtils.isNotNull(whOutboundOrder.getDiscountPrice()) && whOutboundOrder.getDiscountPrice().compareTo(BigDecimal.ZERO) == 1) {//a大于b
            if (basisDiscountPermission.getMaxDiscountPrice().compareTo(whOutboundOrder.getDiscountPrice()) == -1) {
                throw new BusinessException("已超过最大折让权:" + basisDiscountPermission.getMaxDiscountPrice());
            }
        }
